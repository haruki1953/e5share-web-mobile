...
export const useShareStore = defineStore(
  ...
  () => {
    ...
    // 申请分享的返回信息
    const applyInfo = ref<LinkInfo[]>([])
    // 发送确认的返回信息
    const confirmInfo = ref<LinkInfo[]>([])
    ...
    // 根据用户id获取分享信息
    const findShareInfoByUserId = (userId: number) => {
    ...
    // 添加申请信息
    const addApplyInfo = (userId: number, notifUuid: string) => {
      // 检查是否存在相同 userId 的信息
    ...
    // 处理申请信息
    const handleApplyInfo = (info: LinkInfo) => {
    ...
    // 根据用户id获取申请信息
    const findApplyInfoByUserId = (userId: number) => {
    ...
    // 根据uuid获取申请信息
    const findApplyInfoByUuid = (uuid: string) => {
    ...
    // 添加确认信息
    const addConfirmInfo = (userId: number, notifUuid: string) => {
      // 检查是否存在相同 userId 的信息
    ...
    // 处理确认信息
    const handleConfirmInfo = (info: LinkInfo) => {
      // 如果未找到匹配的申请信息，则返回null
      if (!info || !info.time) return null
    ...
    // 根据用户id获取确认信息
    const findConfirmInfoByUserId = (userId: number) => {
    ...
    // 根据uuid获取确认信息
    const findConfirmInfoByUuid = (uuid: string) => {
