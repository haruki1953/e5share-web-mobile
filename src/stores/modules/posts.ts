...
export const usePostsStore = defineStore(
  ...
  () => {
    ...
    // 已读帖子
    const readPostUuid = ref<string[]>([])

    // 关注的动态e5帐号主id
    const followE5ids = ref<number[]>([])

    const profileStore = useProfileStore()
    const usersStore = useUsersStore()

    // 添加关注e5帐号主
    const addFollow = async (e5id: number) => {
      // 缺失必要数据返回
      if (!isCheckData.value) return
      // 不能添加关注则返回
      if (!shouldAddFollow(e5id)) return
    ...
    // 取消关注指定e5帐号主
    const removeFollow = (e5id: number) => {
    ...
    // 是否可以关注
    const shouldFollow = (e5id: number) => {
      // 检查用户是否存在, 不存在直接返回
    ...
    // 是否可以添加关注（不能重复）
    const shouldAddFollow = (e5id: number) => {
      // 已存在则返回
    ...
    const getPostsList = async () => {
      ...
      // 构造一个包含所有异步操作的 Promise 数组
      const promises = profileStore.user?.helping_by_users.map(
        async (e5id: number) => {
          // 请求获取单个e5账号主的动态
    ...
    // 请求获取单个e5账号主的动态
    const getE5Posts = async (e5id: number) => {
    ...
    // 单个e5账号主的未读帖子数
    const unreadCountInE5Posts = (e5id: number) => {
    ...
    // 设置单个e5账号主动态全部已读
    const markE5PostsAsRead = (e5id: number) => {
    ...
    // 根据id获取e5账号主的动态
    const findE5PostsByE5id = (e5id: number) => {
      return postsList.value.find((e5postInfo) => e5postInfo.id === e5id)?.posts
    }

    // 获取e5账号主最近的动态内容
    const getE5PostLastContent = (e5id: number) => {
