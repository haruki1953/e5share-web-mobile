...
import type { ProfileRes, User, UserRes } from '@/types/user'

// 解析用户信息
export const parseUserInfo = (resData: UserRes): User => {
  // 处理头像
  let avatar
  // 拼接头像完整地址（如果有）
  if (resData.avatar) {
...
// 解析用户通知
export const parseUserNotification = (resData: ProfileRes): Notif[] => {
  let notifications = []
  try {
